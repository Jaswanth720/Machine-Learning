...
def tau_cal(x,y,w):
   return (1 - y * (np.dot(x, w)))/ np.square(la.norm(x))
...
def peptrain(x, y, dev_x, dev_y, test_x, test_y):
    ...
    return (train_acc, test_acc, dev_acc, errors, maxiter)
...
if __name__ == '__main__':
    ...
    (train_acc, test_acc, dev_acc ,errors, maxiter) = peptrain(train_x, train_y, dev_x, dev_y, test_x, test_y)
    
    plt.plot(maxiter, errors)
    plt.title('learning curve')
    plt.xlabel('PAiterations')
    plt.ylabel('errors')
    plt.savefig('PAlearningcurve.png')
    plt.close(1)
    ...
    plt.savefig('PAtestaccuracy.png')
    plt.close(1)
    ...
    plt.savefig('PAtrainingcurve.png')
    plt.close(1)